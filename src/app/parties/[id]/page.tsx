...
export default function PartyDetailPage() {
  ...
  // Movie detail view
  if (selectedMovie) {
    return <MovieInParty movie={selectedMovie} party={party} isMember={!!isMember} user={user}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {isPast && (
          <span className="bg-gray-500/10 text-gray-500 px-4 py-2 rounded-xl text-sm font-medium">Завершено</span>
        )}
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          {party.members.map((m) => (
            <Link key={m.user.id} href={`/profile/${m.user.id}`}
              className="flex items-center gap-2 bg-surface-hover border border-border rounded-xl px-4 py-2 hover:border-amber-400/30 transition-colors">
              {m.user.avatar ? (
      ...
      {/* Add movie form for members */}
      {isMember && !isPast && <AddMovieToParty partyId={party.id} onAdded={fetchParty} />}
        ...
        {party.movies.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 stagger-children">
            {party.movies.map((movie) => {
              const avg = movie.votes.length > 0
                ? movie.votes.reduce((s, v) => s + v.rating, 0) / movie.votes.length : 0;
              return (
                <button key={movie.id} onClick={() => setSelectedMovieId(movie.id)}
                  className="glass-card rounded-2xl overflow-hidden text-left transition-all duration-300 poster-hover group">
                  {movie.poster ? (
...
function AddMovieToParty({ partyId, onAdded }: { partyId: string; onAdded: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await fetch(`/api/parties/${partyId}/movies`, {
      ...
      body: JSON.stringify({
        title: title.trim(), year: year || null, description: description.trim() || null,
        poster: poster.trim() || null, trailerUrl: trailerUrl.trim() || null,
      }),
    });
...
function MovieInParty({ movie, party, isMember, user, onBack, onUpdate }:
  { movie: Movie; party: Party; isMember: boolean; user: { userId: string } | null; onBack: () => void; onUpdate: () => void }) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <button onClick={onBack} className="text-amber-400 hover:text-amber-300 transition-colors">← Назад до п'янки</button>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-6">
          <h1 className="text-4xl font-black tracking-tight">
            {movie.title}{movie.year && <span className="text-gray-600 ml-3 text-2xl font-normal">({movie.year})</span>}
          </h1>
          {movie.description && <p className="text-gray-400 leading-relaxed text-lg">{movie.description}</p>}
          {movie.trailerUrl && (
            <div><h2 className="text-lg font-bold mb-3 flex items-center gap-2"><span className="text-amber-400">▶</span> Трейлер</h2>
              <YouTubeEmbed url={movie.trailerUrl} /></div>
          )}
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl p-8">
        ...
        <div className="flex items-end gap-4 mb-6">
          <span className="text-6xl font-black gradient-text">
            {movie.votes.length > 0 ? (movie.votes.reduce((s, v) => s + v.rating, 0) / movie.votes.length).toFixed(1) : "—"}
          </span>
              ...
              <div key={v.userId} className="flex items-center gap-4">
                ...
                <div className="flex gap-1 flex-1 max-w-xs">
                  {Array.from({ length: 10 }, (_, i) => (
                    <div key={i} className={`h-3 flex-1 rounded-full transition-all duration-300 ${
                      i < v.rating ? "bg-gradient-to-r from-amber-500 to-amber-300 rating-bar" : "bg-surface-hover"
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-3 bg-surface-input border border-border rounded-xl px-4 py-3">
              <input type="range" min={1} max={10} value={rating}
                onChange={(e) => setRating(Number(e.target.value))} className="w-32 accent-amber-400" />
      ...
      <div className="flex justify-end">
        <button onClick={handleDelete} className="text-red-400/60 hover:text-red-400 text-sm transition-colors btn-press">Видалити фільм</button>
      </div>
    </div>
  );
}
